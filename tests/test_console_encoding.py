...
진입점이 `force_utf8_output()` 을 안 부르면:
  ...
  3. 컨슈머에서는 그게 `ValueError` 하위라 `consume()` 이 계약 위반으로 분류해
     `nack(requeue=False)` → **DLX. 메시지가 유실된다**(실측)
...
------------------------------------------
초안은 소스를 문자열로 훑었는데 **네 방향으로 뚫렸다**(넷 다 재현됨):
...
**끝 `/` 는 선택이다** — `COPY app ./app` 도 도커에선 같은 뜻인데, 처음에 `/` 를 필수로
   뒀더니 그 형태를 통째로 놓쳤다(자체 검수에서 발견). 유도 집합이 **조용히 좁아지는**
   방향이라, 이 파일이 "Dockerfile 이 바뀌면 가드가 따라온다" 고 주장하는 근거가 깨진다.
파일 이름은 안 잡는다 — `[\\w-]+` 가 점을 못 먹어 `COPY requirements.txt .` 는 매칭에
   실패한다(`requirements` 뒤에 `/` 도 공백도 아닌 `.` 이 온다).
`--from=builder` · `--chown=u:g` 같은 플래그가 붙어도 잡는다.
중첩 경로(`COPY data/config/ ...`)는 안 잡지만, 비교 대상이 최상위 폴더명뿐이라 무해하다.
"""
...
그렇게 걸렸다. 안내 문구를 고쳤다고 가드가 요구사항을 늘리면 안 된다.

범위를 넓힌 뒤로는 앞의 둘이 **다른 사유**(위험 문자)로 어차피 대상이라 이 제외가
   덜 보이지만, `app/core/mq.py` 는 여전히 `__main__` 블록이 없는 라이브러리 모듈이다 —
   주석 제외가 깨지면 그게 요구 대상이 되고, 넣을 자리조차 없어 영영 빨간 테스트가 된다.

처음엔 `^\\s*#` 로 **전체줄만** 지웠는데 `app/core/mq.py` 하나가 계속 남았다 —
...
**exec-form(`["python", "-u", "scripts/x.py"]`)도 잡아야 한다** — 이 compose 가 이미
   그 관용구를 쓰고 있다(`entrypoint: ["/bin/bash", "-c"]` · healthcheck `test:`).
   그래서 앞에 따옴표·대괄호·쉼표가 와도 매칭한다.
`[\\w/]` 만 쓰면 `scripts/my-tool/run.py` 같은 하이픈·점 경로를 놓친다.
`./scripts/foo.py` 의 `./` 는 선택적으로 흡수한다.
"""
...
def _is_main_guard(node: ast.stmt) -> bool:
    ...
    문자열 매칭이 아니라 **AST** 다 — 홑따옴표도 잡고, docstring·주석 안의 언급은
...
def _helper_binding(node: ast.stmt) -> bool | None:
    ...
    **바인딩이지 "언급" 이 아니다.** `from ... import X as Y` 는 `Y` 를 바인딩하고 `X` 는
...
def _imports_canonical_helper(tree: ast.Module) -> bool:
    ...
    **최상위 직속 문장만 본다.** `try:`/`if:` 안에 감춘 재정의는 안 잡는다. 저장소에 0건이고,
       실행 경로가 갈리는 바인딩은 정적으로 판정할 수 없다 — 그런 게 생기면 그때 넓힐 것.
    `import app.core.console` 후 `app.core.console.force_utf8_output()` 형태도 일부러
...
def _calls_helper_first(tree: ast.Module) -> bool:
    ...
    **왜 "어딘가에 있으면" 이 아니라 "첫 문장" 인가 — 실버그로 확인됐다.**
    ...
    앞에 무언가를 꼭 둬야 하는 진입점이 나오면 그때 완화하되 **왜 완화하는지 근거를
...
def _has_uncp949_literal(tree: ast.Module) -> bool:
    ...
    **한글은 cp949 에 있다.** 걸리는 건 `—`(U+2014) · `⚠️` · `ℹ️` · `❌` 같은 것들이라,
...
def _image_source_dirs() -> list[str]:
    ...
    **손으로 적지 않는 이유.** 예전에는 `app/**` 를 코드에 박아뒀는데,
       `COPY scripts/ ./scripts/` 가 늘면서 **그 목록이 조용히 낡았다.** 분류 워커는
...
def _guarded_entrypoints() -> list[Path]:
    ...
    **1 의 근거를 오해하지 말 것 — "컨테이너에서 깨진다" 가 아니다.** 이미지는 리눅스라
    ...
    **두 사유를 합집합으로 두는 이유.** 한쪽이 다른 쪽을 대체하지 않는다 — 출력이 ASCII
    ...
    **여기에 개수를 적지 말 것.** 예전 docstring 이 *"28개"* 와 특정 파일의 줄 번호를
    ...
    `app/main.py` 는 대상이 아니다 — `__main__` 블록이 없고 Dockerfile 이
       `uvicorn app.main:app` 으로 띄운다. 스트림 소유자가 uvicorn 이라 우리가 끼어들
       자리가 아니다. 그 파일의 설정 오류 처리는 별건이다.
    """
    found: set[Path] = set()
...
def test_guarded_entrypoints_switch_console_encoding() -> None:
    ...
    **호출과 import 를 둘 다 본다.** 호출만 보면 `import` 한 줄만 지운 회귀를 놓친다 —
       가드는 통과하는데 CLI 는 첫 문장에서 `NameError` 로 죽는다(실측).
       사유는 `_imports_canonical_helper`.

    **import 는 모듈 최상단, `__main__` 가드보다 앞.** 함수 안에 두면 실행은 되더라도
    ...
    for path in _guarded_entrypoints():
        ...
        if tree is None or not _calls_helper_first(tree):
            no_call.append(rel)
        # 두 목록을 **따로** 모은다. 합쳐 보고하면 어느 쪽이 빠졌는지 몰라 고치는 사람이
...
def test_the_derivation_actually_finds_the_known_entrypoints() -> None:
    ...
    #    특히 `classification_worker.py` 는 k8s CronJob 으로 배포되는데,
...
**대장이 아니라 표본이다.** 전수 검사는 위 `test_guarded_entrypoints_...` 가 AST 로 한다.
...
def test_sampled_entrypoints_survive_cp949_help(rel: str) -> None:
    ...
    **구조 가드와 역할이 다르다.** 위 `test_guarded_entrypoints_...` 는 AST 로
    ...
    **`text=True` 를 쓰지 말 것 — bytes 로 받는다.** 자식은 `force_utf8_output()`
       ...
       0 그대로라 단언은 통과하면서 출력만 사라진다 — `test_generate_determinism`
       에서 고친 바로 그 버그다.
    `env` 를 통째로 교체하지 않고 `os.environ` 을 물려준다. 교체하면 자식이 `PATH` 를
       잃는다(같은 건).
    """
    proc = subprocess.run(
...
# monkeypatch 배선 테스트(`module.main()` 을 직접 호출)는 여기서 지웠다.
...
"""`demo_sim` 계열 모양 — 옛 규칙은 `AsyncFunctionDef` 를 못 봤다."""
...
# 아래 넷은 "정본 import 가 어딘가 있으면 통과" 를 뚫는다(넷 다 재현됨).
ALIASED_IMPORT = (
...
def test_import_detector_requires_the_canonical_helper_import(
    ...
    마지막 두 케이스가 *"존재하면 통과"* 와 *"마지막 바인딩이 정본"* 을 가른다 —
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # 아래 셋이 새 `_entry_body` 규칙의 핵심이다(확인된 실제 모양).
        (
            ASYNC_DELEGATION,
            True,
...
def test_dockerfile_copy_forms_are_understood(line: str, expected: list[str]) -> None:
    ...
    이게 없으면 파싱이 조용히 좁아져도 `_guarded_entrypoints()` 가 그냥 작아질 뿐이라
...
def test_the_arrow_is_encodable_so_only_one_char_per_line_blows_up() -> None:
    ...
    초안은 이 단언을 위 파라미터라이즈 테스트 끝에 **assert 없는 맨 표현식**으로
       뒀는데, 그러면 지워져도 아무것도 안 깨진다.

    바이트값을 박지 않는다 — 처음에 손으로 적었다가 틀렸다(`\\xa1\\xe7` 인데
