...
⚠️ **`sslmode=disable` 을 붙인다** — compose 는 SSL 을 안 켜는데 우리 기본값이 `require` 다.
...
⚠️ LLM·네트워크 없음. 분류기는 스텁이고, 표식 접두사가 붙은 행만 넣고 지운다.
"""
...
def pg(monkeypatch):
    ...
    ⚠️ 지운 뒤 우리 DDL 로 되세우므로 다른 게이트 테스트에는 같은 모양이 남는다(뷰·제약
       포함). 08-18 확정대로 **AI 소유 4개 + 뷰는 우리가 만드는 것**이라 이게 정본이다.

    ⚠️ 접속 원자값 세팅이 `tests/test_raw_db_postgres.py` 와 겹친다 — 일부러 복제했다.
...
def test_source_ddl_is_valid_postgres(pg):
    ...
    ⚠️ 운영에서는 이 DDL 이 안 돈다(그 6개는 인프라·BE 소유다) — 그래도 재는 이유는
    ...
    ⚠️ 별도 스키마에 만들고 지운다 — `public` 에 대고 하면 `IF NOT EXISTS` 가 조용히
...
def test_legacy_scan_catches_a_missing_unique_constraint_on_postgres(pg):
    ...
    ⚠️ 이 상태는 지어낸 것이 아니다 — 인프라가 낡은 문서로 먼저 세워 두면
...
def test_create_classified_tables_survives_concurrent_runs(pg):
    ...
    ⚠️ 창이 좁아서 이 테스트가 **매번 충돌을 재현하지는 않는다.** 여기서 보는 것은
...
def test_review_mixed_signal_is_stored_as_boolean(pg):
    ...
    ⚠️ **CS 로는 이 경로가 안 걸린다.** CS 는 `mixed_signal` 이 항상 `None` 이라
...
def test_monthly_aggregate_reads_postgres(pg):
    ...
    ⚠️ 여기서 갈리는 것은 **행 접근 방식**이다. 예전에는 `for aspect, sentiment, count in
