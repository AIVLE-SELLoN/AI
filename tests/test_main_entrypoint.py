...
`app/consumer.py` 와 같은 계약을 웹 쪽에도 건다. 다만 **모양이 다르다** —
...
def _free_port() -> int:
    ...
    고정 포트를 박으면 두 방향으로 샌다 — 그 포트가 이미 쓰이면 **무관한 이유로**
       실패하고, 회귀로 서버가 정말 뜨면 그 포트를 점유한다.
    """
...
여유를 크게 잡는 게 맞는 이유는 **실패 방향이 비대칭**이라서다. 우리가 잡으려는 회귀는
...
def _launch(argv: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    `encoding="utf-8"` 을 명시한다 — 자식이 한글을 내는데 부모가 자기 locale 로
       디코드하면 mojibake 나 `UnicodeDecodeError` 가 난다.
    `env` 를 통째로 교체하지 않는다 — 교체하면 자식이 `PATH` 를 잃는다(같은 건).
    """
...
def test_config_failures_exit_as_config_error(
    ...
    **두 갈래를 다 돈다.** 이 분기가 덮는 실패는 둘인데(로깅 설정 / 설정 로딩) 한쪽만
       재면, 누가 `settings = get_settings()` 를 `try` 위로 올리는 "정리" 를 해도
       스위트가 초록이고 `MQ_PORT` 오타가 다시 exit 1 이 된다.

    **`get_settings` 를 갈아끼운다 — `basicConfig` 가 아니다.** `basicConfig` 를
...
def test_the_message_never_echoes_the_offending_value(monkeypatch, capsys):
    ...
    리뷰에서 *"원값도 넣자"* 는 제안이 있었는데 이 사유로 안 넣었다.
    """
    monkeypatch.setattr(logging.root, "handlers", [])
...
def test_valid_config_applies_the_configured_level(monkeypatch):
    ...
    이게 없으면 `configure_logging_or_exit()` 이 **아무것도 안 하게** 바뀌어도(예: try
...
def test_the_guard_survives_a_preexisting_root_handler(
    ...
    해법이 `force=True` 가 **아닌** 이유: 그건 기존 핸들러를 지우며 `close()` 까지
...
def test_entrypoints_do_not_declare_their_own_exit_codes():
    ...
    쓰기 시작하는 것**으로 온다 — 정의가 한 곳뿐이면 값 비교 단언은 동어반복이라
    그걸 못 본다.
...
def test_the_contract_values_are_what_k8s_expects():
    ...
    *"두 진입점의 값이 같은지"* 를 재는 단언은 **동어반복이라 넣지 않았다** — 정의가
       한 곳뿐이라 `==` 도 `is` 도 갈릴 수가 없다. 진짜 위험인
...
def test_the_exit_code_survives_the_real_launcher(argv, why):
    ...
    **`--reload`·`--workers` 는 일부러 안 쓴다.** 그 형태는 부모가 감독 프로세스라
    ...
    **`설정을 읽지 못해` 를 반드시 본다.** 종료코드만 보면 **다른 이유로도 초록**이
    ...
    포트는 OS 에게 빈 것을 받는다. 고정 포트를 박으면 이미 쓰이는 경우 **무관한 이유로**
       실패하고, 회귀로 서버가 정말 뜨면 그 포트를 점유한다.
    """
