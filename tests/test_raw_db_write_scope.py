...
여기 두 가드는 성격이 다르다. 합치지 말 것:
  ...
  ② 쓰기 대상 고정   분류 워커가 **AI 소유 4개 밖**에 쓰면 걸린다.
                     → 인프라가 RW 전면 부여로 회신해(2026-08-18) DB 가 더는 안 막는다.
...
⚠️ LLM·네트워크·DB 없음. 소스를 AST 로 읽기만 한다.
"""
...
⚠️ `psycopg.Error` 는 `FileNotFoundError` 도 `RuntimeError` 도 `OSError` 도 **아니다** —
   기존 분기를 그냥 통과한다. 목록은 `raw_db.connection_error_types()` 가 준다.
   (용준님 PR #101 후속 ①: *"ⓑ 에서 워커가 세 번째 호출부가 된다"*)
"""
...
def test_raw_db_call_sites_handle_connection_errors(module: str):
    ...
    ⚠️ **약한 가드다** — 이름을 언급하기만 해도 통과한다(예외를 실제로 잡는지는 안 본다).
...
def _sql_literals(path: Path) -> list[str]:
    ...
    ⚠️ 소스 전체를 정규식으로 훑으면 안 된다 — 이 저장소는 주석·docstring 에 SQL 을
...
def test_worker_only_writes_to_ai_owned_tables():
    ...
    **DB 가 더는 안 막아 준다.** 인프라가 *"모든 테이블에 대한 RW 권한을 기능마다 따로
    부여할 순 없다"* 며 AI 노드에 raw DB **RW 전면 부여**로 회신했다(2026-08-18) — 계정
...
def test_worker_ddl_only_creates_ai_owned_objects():
    ...
    ⚠️ 쓰기 문장과 **다른 축이다.** `create_source_tables()` 는 main server 6개를 만드는데,
