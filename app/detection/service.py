...
⚠️ 입력 의존성 — 백엔드와의 계약:
  ...
  단, master_sku_code 의 '레벨'이 문서 간에 어긋나 있다:
    - 매핑 문서: 옵션 레벨 (MSKU-001 → C100-BLK-M)
    - 이상탐지 로직 §7-2: **상품 그룹 레벨** ← 이 코드는 이 해석을 따른다
  detection 은 상품×채널로 집계하므로 **상품 그룹 레벨**이어야 한다.
  백엔드가 옵션 레벨 ID 를 넘겨주면 집계가 불가능하니, 스키마 회의에서 확정 필요.
"""