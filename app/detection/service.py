...
입력 계약: 상품 매핑은 백엔드 담당이고 detection 은 그 산출물인 `product_group_id` 를 받는다.
이 값은 옵션(SKU)이 아니라 상품 그룹 레벨이며 회사별 시퀀스다. 문의·리뷰 원본에 옵션 정보가
없어 옵션 레벨 ID 로는 집계가 성립하지 않는다. 회사 범위의 고유성은 envelope 의 companyId 와
백엔드 `(companyId, alert_id)` 복합 유니크 계약이 담당한다.
