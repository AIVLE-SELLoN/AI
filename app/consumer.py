...
**Git Bash 에서 `MQ_VHOST=/` 를 인라인으로 주면 안 된다.** MSYS 경로 변환이 `/` 를
...
def wire_handlers() -> None:
    ...
    **이걸 안 부르면 `HANDLERS` 가 비어 있어 모든 이벤트가 DLX 로 간다.** 컨슈머를 직접
    띄우지 않고 `dispatch()` 만 쓰는 도구(스모크 스크립트 등)도 이 함수를 먼저 불러야 한다 —
    공개 함수인 이유가 그것이다.

    등록 안 된 이벤트는 ACK 하지 않고 DLX 로 보낸다 — 처리한 적 없는 걸 처리했다고 하면 그
    메시지가 사라진다. 인바운드 2종(`docs/mq_events.md` §8)이 여기 다 꽂혀 있어야 한다.

    import 를 **함수 안에서** 한다. 모듈 최상단으로 올리면 컨슈머를 띄우지 않는 테스트·도구
    까지 recommendation·reporting 을 끌고 들어온다.
    """
    ...
    # `mq_consumer.HANDLERS` 를 **모듈 경유로** 읽는다. `from … import HANDLERS` 로 가져오면
    # import 시점의 객체가 이 네임스페이스에 값으로 묶여, 테스트가
    # `monkeypatch.setattr(mq_consumer, "HANDLERS", {})` 로 갈아끼웠을 때 등록은 새 dict 에
    # 되고 이 로그는 옛 dict 를 읽어 "비었다"고 말한다.
...
def main() -> None:
    ...
    # 빠지면 조용히 사라지는 게 아니라 **시끄럽게 잃는다** — 진단 경고 대신
    # `--- Logging error ---` 블록이 건당 10줄 쌓인다. 게다가 단일행 warning
    # (`_format_submitted_at`)은 **호출부로 탈출해 메시지까지 잃는다**: `UnicodeEncodeError` 가
    # `ValueError` 하위라 `consume()` 이 계약 위반으로 보고 `nack(requeue=False)` → DLX.
    force_utf8_output()

    # **설정 로딩·로깅 설정이 여기(try 안)에 있어야 한다.** 밖에 두면 `LOG_LEVEL` 오타 하나가
    # 미포착 예외로 나가 exit **1**(일시적 오류)이 된다 — 재시작해도 같은 값을 읽으니 k8s 는
    # 영원히 재시작만 하고 아무도 못 알아챈다. 처리 본체는 `app/core/logging_setup.py` 한 곳에
    # 있다(진입점마다 같은 블록을 복사하면 진단 문구 결함을 그 수만큼 고쳐야 한다).
