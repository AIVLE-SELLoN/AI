...
BH_FDR_Q = 0.05
"""[관문②] BH-FDR의 목표 FDR 수준.

일별 이상탐지는 상품별 family(`app.detection.statistics`), 월간 JSD는 전체
(상품 x 채널쌍) family(`app.reporting.metrics_calculator`, 리포팅 로직 §4-2)에 같은
q값을 적용한다. 독립 또는 양의 의존 등 BH 가정 아래에서 각 소비자가 정의한 family의
기대 거짓발견률을 제어한다. 한 번의 실행에서 최종 발행 알림의 헛알림률이 항상 5%
이하라는 뜻은 아니다. 결합·억제·채점 정책을 거친 데모 헛알림률은 별도 실측 지표다.
