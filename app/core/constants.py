...
NOTICE_MAX_LISTED_PRODUCTS = 5
"""콜백 `notice_message` 에 **이름을 나열할** 상품 수 상한. 넘으면 "외 N개" 로 줄인다.

⚠️ 상한이 없으면 길이가 상품 수에 비례해 자란다 — 실측으로 보류 42건이면 631자,
   보류 42 + 실패 10 이면 742자다(2026-08-09). `notice_message` 에는 스키마 max_length 가
   없어서 우리 쪽에서 안 걸리고, **백엔드 컬럼이 VARCHAR(255) 같으면 조용히 잘리거나
   INSERT 가 터진다.** 어느 쪽이든 셀러는 자기 상품이 왜 빠졌는지 못 본다.

5개로 잡은 이유: 상세는 이제 **합본 PDF 의 보류 페이지**에 상품마다 있다(2026-08-09).
안내 문구는 "무엇이 몇 개 빠졌는지" 를 알리는 용도라 전부 나열할 이유가 없다.
이 값이면 보류·실패가 동시에 최악이어도 약 260자다.
