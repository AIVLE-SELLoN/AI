...
NOTICE_MAX_LIST_CHARS = 70
"""콜백 `notice_message` 에서 **상품 이름 나열에 쓸 글자 수** 상한(구절 하나당).
넘으면 거기서 끊고 "외 N개" 로 줄인다.

⚠️ 개수가 아니라 **길이**로 자른다. 개수로 자르면 상품명이 길 때 상한이 안 지켜진다 —
   `_fetch_product_names()` 가 `products.channel_product_name`(쿠팡·네이버 노출명)을
   자르지 않고 그대로 싣는데, 그게 원래 길다:
       "2026 신상 봄가을 여성 미디 원피스 데일리 롱 A라인 5color"  → 38자
   개수 상한(5개)만 걸었을 때 실측(2026-08-09):
       목 데이터 이름(7자)  보류42+실패42 = 223자   ← 로컬에서는 안 밟힌다
       실제 노출명(38자)    보류42+실패42 = **368자**  ← VARCHAR(255) 초과
   목 데이터가 짧아서 개수 상한이 통하는 것처럼 보였을 뿐이다.

이 값이면 상품명이 아무리 길어도 구절 하나가 대략 130자, 보류·실패가 동시에 최악이어도
**270자 아래**로 유지된다(고정 문구 포함).
...
⚠️ 백엔드 컬럼 길이를 아직 확인 못 했다. 알게 되면 이 값을 그 기준으로 다시 잡는다.
"""
