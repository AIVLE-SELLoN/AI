...
🔴 **아래 DDL 중 AI 소유 4개 + `voc_document` 뷰는 운영에서도 쓰인다(2026-08-18 확정).**
   ⚠️ 2026-08-16 에 이 자리에 *"이 DDL 은 운영에서 안 쓰인다"* 라고 적어 뒀는데 **거짓이
   됐다.** 인프라(민준) 확인 결과 **AI 소유 테이블과 우리 읽기 모델은 우리가 만든다** —
   워커 첫 실행의 `create_classified_tables()` 가 그것이고, 그래서 인프라 요청 3건
   (뷰·UNIQUE·버전 인덱스)이 통째로 철회됐다. 갈리는 것은 소유(§1)와 같다:

       main server 6개 (channel·products·mapped_data·orders·cs·reviews)  → 인프라·BE
       AI 소유 4개 + voc_document 뷰                                      → **우리**

   그래서 `create_source_tables()` 만 로컬·목 전용이고, `create_classified_tables()` 는
   운영 경로다. 여기 DDL 을 고치면 운영 스키마가 바뀐다.

🔴 **DDL 은 백엔드 2종을 **갈라서** 낸다 — 한쪽으로 통일하지 말 것.** 데모(sqlite
   `data/raw.db`)가 그대로 돌아야 하고 운영은 Postgres 다. 문법이 실제로 갈리는 곳:
   ...
   Postgres 쪽 타입은 노션 「RAW DB 스키마」(DDL 전문, DBML)를 그대로 따른다 — 인프라가
   먼저 세워 뒀을 수 있고, 그때 `CREATE TABLE IF NOT EXISTS` 는 조용히 건너뛰므로
   **우리 정의가 문서와 갈리면 그 갈림이 영원히 안 드러난다.**
   ⚠️ 두 정의가 같은 테이블·컬럼을 내는지는 `tests/test_raw_db_dialects.py` 가 잡는다.

⚠️ 시각은 전부 **오프셋이 붙은 ISO 문자열**로 넣는다 — §3 이 날짜 경계를 Asia/Seoul 로
   못박았는데, 오프셋 없이 넣으면 TIMESTAMPTZ 인 운영 컬럼에서 어느 지역 시각인지 알 수
   없어 하루가 밀린다.
"""
...
# 갈리면 목에서만 돌거나 운영에서만 도는 컬럼이 생기는데, `CREATE TABLE IF NOT EXISTS`
# 라 이미 있는 쪽은 조용히 넘어가서 **그 갈림이 조회 단계에 가서야 드러난다.**
# `tests/test_raw_schema_dialects.py` 가 두 판을 대조한다.
#
# 타입은 노션 「RAW DB 스키마」(DDL 전문, DBML) 그대로다.
# ⚠️ `VARCHAR(n)` 은 넘치면 **Postgres 에서만** 적재가 실패한다(sqlite 의 TEXT 는 안 걸린다).
#    지금 넣는 값은 여유가 크지만(`classify_sentiment_v4` 21자 / VARCHAR(30)), 버전 문자열을
#    길게 지을 때 이 한도를 같이 볼 것.
...
# ⚠️ `AUTOINCREMENT` 는 Postgres 에 없다. `GENERATED BY DEFAULT`(ALWAYS 아님)인 이유는
#    골든 주입처럼 id 를 명시해 넣는 경로가 생겨도 막히지 않게 하려는 것이다.
# ⚠️ `mixed_signal` 은 BOOLEAN 이다 — sqlite 는 INTEGER 라 적재 쪽이 `int()` 로 캐스팅하면
#    Postgres 에서 타입 오류가 난다. 파이썬 `bool` 을 그대로 넘겨야 양쪽이 다 받는다.
...
# ⚠️ Postgres 에 `CREATE VIEW IF NOT EXISTS` 는 없다. `OR REPLACE` 는 컬럼 이름·순서·타입이
#    같을 때만 통과하므로, 뷰 정의를 바꾸면 여기서 `cannot change name of view column` 으로
#    시끄럽게 막힌다 — 조용히 옛 뷰가 남는 것보다 낫다.
...
def create_source_tables(conn) -> None:
    ...
    ⚠️ **운영에서는 안 불린다** — 이 6개는 인프라·BE 소유다(§1). 목 파이프라인의
       `mock_producer` 가 그 자리를 대신할 때만 돈다.

    ⚠️ `products` 는 `channel` 다음, `mapped_data` 는 `products` 다음이어야 한다 —
       FK 가 그 방향으로 걸려 있다. 다만 **sqlite 에서는 깨지는 곳이 CREATE 가 아니라
       적재다.** 부모 테이블이 없어도 `CREATE TABLE ... REFERENCES` 를 그냥 통과시키고
       (실측: 예외 없음), 그 다음 INSERT 에서 `no such table: main.products` 로 터진다.
       **Postgres 는 CREATE 단계에서 바로 막는다** — 순서가 계약인 것은 양쪽 같고 드러나는
       시점만 다르다.
    """
...
def create_classified_tables(conn) -> None:
    ...
    (프로듀서 없이 덤프만 받은 상태), 뷰가 없으면 조회가 통째로 실패한다. 운영에서도
    같은 이유로 우리가 만든다(2026-08-18 확정 — 인프라 요청 3건 철회).

    ⚠️ `IF NOT EXISTS` 라 **이미 있는 테이블은 손대지 않는다.** 8/7 확정 이전 구조로
       남아 있거나 인프라가 낡은 문서로 먼저 세운 DB 에 대고 부르면 조용히 통과하므로,
       호출 전에 `find_legacy_tables()` 로 먼저 걸러야 한다.

    ⚠️ **동시 실행에 견딘다** — `raw_db.execute_ddl()` 이 Postgres 의 동시 CREATE 충돌을
       삼킨다. 워커가 CronJob 이라 겹쳐 뜰 수 있고, 그때 진 쪽이 죽으면 무한 재시도가 된다.
    """
    dialect = raw_db.dialect_of(conn)
...
# 컬럼만으로는 못 잡는 것 — **유니크 제약**.
#
# 🔴 컬럼이 다 있어도 이 제약이 없으면 재분류가 같은 `(item_id, aspect)` 를 **중복 적재**한다.
#    워커는 2026-08-12 부터 "aspect 는 지우고 다시 넣는다" 로 도는데, 그 설계가 이 제약을
#    전제로 짜여 있다(`INSERT ... ON CONFLICT DO NOTHING` 이 한 응답 안의 중복만 막는 것도
#    여기 의존한다). 없으면 탐지 분자가 부풀고, 그건 **오탐 방향이라 시끄럽지도 않다.**
#
# ⚠️ 이 가드가 필요해진 이유가 08-18 에 바뀌었다. 예전에는 "인프라가 세우니 요청해야 한다"
#    였는데, 지금은 **우리가 만든다.** 그래도 남는 위험이 있다 — 인프라가 낡은 문서로
#    `classified_item_aspect` 를 먼저 세워 뒀으면 `CREATE TABLE IF NOT EXISTS` 가 조용히
#    건너뛰고 제약 없는 옛 모양이 남는다. 그 상태를 여기서 잡는다.
...
def find_legacy_tables(conn) -> list[str]:
    ...
    두 축을 본다:
      1. **컬럼** — `LEGACY_MARKERS` 의 마커 컬럼이 없으면 옛것.
      2. **유니크 제약** — `REQUIRED_UNIQUE_COLUMNS` 의 조합이 없으면 옛것.

    없는 테이블은 대상이 아니다 — 조회가 빈 결과를 주고, 그건 새로 만들면 되는 정상
    상태다. "있는데 모양이 옛것"인 경우만 골라낸다.

    ⚠️ 스키마 조회를 `raw_db` 에 맡긴다 — sqlite 는 `PRAGMA`, Postgres 는
       `information_schema`·`pg_index` 로 문법이 완전히 갈리는 자리다. 여기서 `PRAGMA` 를
       직접 쓰면 **Postgres 에서 이 가드 자신이 구문 오류로 죽는다**(스키마가 멀쩡한지
       보려던 코드가 먼저 터지는 모양이라 원인이 메시지에 안 드러난다).
    """
    stale = []
