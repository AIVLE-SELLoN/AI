...
⚠️ **세 번째 진입점(`app/batch/daily.py`)은 아직 이 함수를 안 쓴다.** 거긴 로깅 레벨을
   `logging.INFO` 로 하드코딩해서 `LOG_LEVEL` 오타에는 애초에 안 걸리지만,
   `get_settings()` 가 `run_batch` 안쪽에서 불려서 **`MQ_PORT=abc` 같은 값 오류는
   여전히 exit 1 + raw traceback** 이다(실측). 배치는 CronJob 이라 같은 문제가 있는데,
   그쪽은 실패 지점이 부팅이 아니라 실행 중이라 이 함수 모양으로는 안 덮인다.
   **셋 중 둘만 이 계약을 쓴다는 뜻이다.**
"""
