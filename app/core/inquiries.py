...
**입력원이 두 개고 둘 다 남는다.** `item_id` 가 `cs.id`/`reviews.id` 그대로라 조인이
1컬럼이다.
...
(`load_inputs_from_db`) `evidence.inquiry_ids` 는 항상 그 안(현재 윈도우)이라, 조회로
바꾸면 **같은 행을 두 번 읽는다.** 정책(순서·중복·빈 원문 처리)이 갈릴 걱정은 없다 —
`fetch_linked_inquiries` 가 `build_linked_inquiries` 를 그대로 부른다.
"""
...
def build_linked_inquiries(
    ...
    **`source` 값이 틀려도 그 문의는 살린다 — 출처 미상(None)으로 둘 뿐이다.**
    ...
    1건 때문에 그 알림의 문의가 통째로 사라진다**(문서 못 찾음이면 1건만 빠지는 것과
    대비된다).
...
def fetch_linked_inquiries(
    ...
    *,
    ...
    원문의 정본은 원본 DB 이고 AI 노드는 읽기 권한이 있다. `item_id` 가 `cs.id`/`reviews.id`
    그대로라 조인이 1컬럼이다.

    **`cs.created_at` 이 아니라 `cs.inquired_at` 이다.** `cs` 에는 문의 발생 시각과 레코드
    적재 시각이 **둘 다** 있고 `LinkedCSInquiry.created_at` 의 정의는 "CS 접수 일시" 라
    전자가 맞다 — 이름이 같아서 그냥 매핑하면 조용히 틀린 값이 들어간다. `voc_document`
    뷰가 두 소스의 시각을 `occurred_at` 하나로 맞춰 두므로 그 뷰를 쓴다.

    **리뷰(`RVW-`)도 딸려 온다 — 그게 확정 정책이다.** 국내 커머스는 셀러가 리뷰에 답글을
    달고 그것도 CS 업무라, 리뷰 전용 알림의 가이드라인을 버릴 이유가 없다. 구분이 필요한
    쪽을 위해 `LinkedCSInquiry.source` 로 출처를 실어 보낸다.
    ...
    **거르는 쪽으로 되돌리려면 `is_guideline_target()`(`app/reporting/cs_reply_service.py`)
    ...
    `ValueError` 로 죽는다 — **정상 동작이 배치 요약의 "진짜 실패"로 집계된다.**
