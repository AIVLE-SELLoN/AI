"""프롬프트 파일 로더. 4개 모듈이 같은 로직을 필요로 해서 core 에 둔다.

프롬프트는 코드에 하드코딩하지 않고 `prompts/` 별도 파일로 둔다(협업 규칙 2). 버전 비교
실험을 해야 하므로 파일명의 v1/v2 를 인자로 받는다.

**프롬프트 파일에는 프롬프트만 넣는다.** 입출력 계약·후처리·미결사항 같은 설계 메모는
service.py docstring 으로. `load_prompt` 는 파일 전체를 그대로 반환하므로 메모를 같이
넣으면 그게 전부 LLM 에 전송된다.

**str.format() 을 쓰지 말 것.** 우리 프롬프트는 출력 형식·few-shot 예시로 JSON 을 가지고
있어서 format() 이 JSON 의 중괄호를 플레이스홀더로 오해하고 KeyError 로 터진다.
`string.Template` 과 `$플레이스홀더`를 쓴다:
...
원문 텍스트를 끼워 넣을 때는 반드시 `json.dumps` 로 직렬화할 것 — 따옴표·줄바꿈이 든 CS
원문을 그대로 넣으면 프롬프트의 JSON 이 깨진다. `substitute()` 는 못 채운 플레이스홀더가
있으면 KeyError 를 내므로 오타를 조용히 넘기는 `safe_substitute()` 보다 낫다.
"""
