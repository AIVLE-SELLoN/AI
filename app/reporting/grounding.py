...
검사는 둘이다: 수치 팩트체크(LLM 이 쓴 `13%`·`450건`·`0.54` 가 입력에 실제로 있는 수치
인가)와 금지 표현(`p-value`·`FDR` 같은 통계 용어가 셀러용 문서에 새어나갔는가).

검사 대상 필드는 호출부가 고른다. `standard_guideline.*` 처럼 정책 상수(무상 교환 기간
등)가 들어가는 필드는 넘기지 않는다 — 넘기면 입력에 없는 숫자라고 전부 반려된다.
"""
...
#
# 천단위 콤마를 반드시 받아야 한다. 없으면 `1,200건` 이 `200건` 으로 잘려 맞는 문장을
# 반려하고(1200 이 허용집합인데 200 을 비교) 틀린 문장을 통과시킨다(허용집합에 200 이
# 있으면 승인). 표지 템플릿도 콤마 표기를 쓴다.
