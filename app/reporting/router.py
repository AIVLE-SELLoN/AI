...
classification/detection/recommendation 을 import 하지 않고 core/ 만 쓴다.

응답은 `GenerationCallback` 이다. 지금은 콜백을 응답 본문으로 돌려준다 — 계약상으로는
FastAPI 가 Spring Boot 의 `POST /api/v1/internal/reports/complete` 로 푸시해야 하지만,
그 URL 이 `app/config.py`(공유 영역)에 환경변수로 들어가야 해서 팀 합의 후 붙인다.
...
async def create_report(input_data: MonthlyReportInput) -> MonthlyReportOutput:
    ...
    PDF 는 여기서 만들지 않는다. 운영 산출물은 월 1개 합본이라
    `scripts/generate_monthly_reports.py` 배치가 전 상품을 모아 만든다.

    표본 부족(total_voc_count < 10)이면 LLM 을 태우지 않고 409 로 알린다 — 실패가
    아니라 정상적인 보류 상태다.
    """
...
async def create_reply(input_data: CSGuidelineInput) -> GenerationCallback:
    ...
    `verdict='정상'` 인 알림은 스키마 단계에서 422 로 거른다 — 생성 대상이 아니다.
    """
