...
## 데이터
알림 $alert_id · $product_group_id / $product_name · $channel · $main_aspect · $verdict · 확신도 $detection_confidence · 권장조치 $recommended_action
[지표] $stats_summary
...
## 출력 형식
```json
{"guideline_id":"GD-20260528-P001","alert_id":"$alert_id",
...
```
`guideline_id` = `GD-{탐지일 YYYYMMDD}-{product_group_id}`, `risk_level`은 CRITICAL/WARNING/NORMAL 중 하나.
