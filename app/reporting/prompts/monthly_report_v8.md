...
4. `aspect_summaries`는 **정확히 3건** — 색상·사이즈·소재 각 1건이다. [데이터]의 속성표에
   세 줄이 모두 있으니 하나도 빠뜨리지 마라. **2건 이하로 내면 스키마 위반으로 반려된다.**
   상태가 RISK면 변동폭(%p)을 반드시 쓴다.
...
"aspect_summaries":[{"aspect":"색상","summary_text":"부정 의견이 전월 대비 <속성표의 변동%p>%p 올라 <속성표의 부정%>%를 기록했습니다."},{"aspect":"사이즈","summary_text":"<속성표의 사이즈 행을 근거로 한 문장>"},{"aspect":"소재","summary_text":"<속성표의 소재 행을 근거로 한 문장>"}],
...
"cause_analysis_results":["색상 부정 의견이 전체 <총 VOC 건수>건 중 비중이 가장 큽니다.","<속성표를 근거로 한 두 번째 결론>"],
"recommended_actions":["<한 채널> 대표 이미지를 원본 색상 기준으로 교체하세요.","<두 번째 조치>"]}
```
