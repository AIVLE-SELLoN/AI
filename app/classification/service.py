...
def _cs_empty_fallback(trace_key: str) -> list[AspectSentiment]:
    ...
    현재 coverage 는 aspect 자식이 아니라 분류 부모 행의 존재로 성공을 판단하므로,
    빈 배열 자체가 coverage 미달을 만들지는 않는다. 이 폴백은 프롬프트1의 "CS는 하나
    이상의 aspect" 계약과 ClassifiedItem 출력 형태를 유지하고, 빈 응답을 명시적인
    기타/중립으로 정규화하기 위한 것이다. 반대로 LlmParseError 로 던지면 부모 행이
    만들어지지 않아 실제 coverage 구멍과 재처리 비용이 생긴다.

    '기타/중립'은 어느 탐지 대상 aspect의 분자도 늘리지 않아 가장 보수적이다.
