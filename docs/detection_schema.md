...
> 🟢 **문서 상태: [확정]** — 상품 그룹 ID 형식이 `P{숫자}` 로 확정되면서 마지막 대기 항목이 닫혔다. 코드가 이 계약대로 발행한다(`app/core/schemas.py` `DetectionAlert`). 변경은 양측 합의 + 변경 내역 기록.
> 
...
| verdict | verdict | ✓ | golden 에서 보류 값 제거 완료. 값 목록 동일 |
