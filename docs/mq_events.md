...
`model`은 Agent1의 속성·감성 분류 모델이다. Agent2 `[6]` 원인분류 모델은
`classifier_versions`에 포함되지 않는다. 현재 원인분류는 별도 `CAUSE_LLM_MODEL`을
사용하므로, 소비 측은 이 필드만 보고 `root_cause`의 생성 모델을 추론하면 안 된다.

**왜 필요한가.** 탐지는 35일(현재 7 + 과거 28)을 한 번에 읽는다. 그 사이 분류기가
바뀌면 한 검정 안에 두 라벨러의 결과가 섞여, **분류기 개선이 고객 이상 알림으로 발화한다.**
AI 쪽은 활성 버전 행만 읽고, 윈도우에 옛 버전 행이 **하나라도 있으면 배치를 세운다**
(fail-closed). 따라서 알림이 나갔다는 것 자체가 "기여한 모든 행이 이 버전"이라는 증거다.
다만 그 사실은 payload 에 적히지 않으면 소비 측이 알 길이 없다.

**축이 셋인 이유.** 프롬프트가 그대로여도 라벨러는 바뀐다 — 모델을 갈아끼우면 같은
프롬프트로 다른 라벨이 나오고, **허용 aspect 집합을 바꾸거나 후처리·정규화를 손보면**
안 내던 라벨이 나오거나 라벨이 다른 값으로 정규화돼 집계가 갈린다. 셋 중 하나만 달라도
비교 가능성이 깨진다.
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `guideline_id` | string | **멱등 키.** `alert_id`의 `ALT-` 접두어를 `GD-`로 바꾼 값 — 예: `ALT-20260828-P001-COLOR-COUPANG` → `GD-20260828-P001-COLOR-COUPANG`. 알림과 **1:1**이라 재생성해도 같은 ID<br>⚠️ 백엔드 문서의 `GD-{탐지일}-{상품그룹}`은 **폐기된 규칙**이다. 탐지가 (상품, aspect, 채널) 단위로 발화하므로 같은 날 같은 상품의 다른 알림이 전부 같은 ID가 됐고, 멱등 upsert 때문에 나중 가이드라인이 앞의 것을 조용히 덮어썼다 (PR #22에서 수정, `app/reporting/ids.py`) |
...
3. ~~「메시지 큐 컨벤션 정의」 §4.6의 `guideline_id` 형식이 `GD-{탐지일}-{상품그룹}`~~
   → **해소됨.** 노션 쪽이 `alert_id` 파생으로 갱신됐다(그 절의 번호도 §4.6이 아니라
   **§4.5**다 — 이 항목이 마지막까지 옛 번호를 들고 있었다). 규칙 자체는 §6 참고.
