...
번호 근거: ①~④(mock 정의서) + ⑤ RAG 베이스라인 비교(개선안 로직).
   ⑤가 이미 RAG 실험이라 원인분류는 ⑥. 지인 승인 완료.
...
이 실험은 detection_confidence(신뢰도 출력)와 다르다.
   - 여기: 원인 라벨이 '맞았나'를 채점 = 성능 평가
   - decide_confidence(): 원인이 '일관됐나'를 재료로 신뢰도를 정함 = 런타임 출력
   섞지 말 것.

스코프: 색상/사이즈/소재만 채점한다. **파손·오배송은 제외**(이상탐지 로직에서
   원인 후보 미정의·개선안 없음). 골든에 그 aspect 로 원인 라벨이 붙어 있어도 버린다.

시험지 유출 금지. 프롬프트3의 few-shot 예시는 평가셋 밖에서 사람이 작성한다.
...
def main() -> None:
    # 첫 문장이어야 한다. 이 파일은 `--help` 가 원래 통과한다 — `description` 이
