...
계획B 채택(현진↔팀 논의 결과) — 71630 원본 sentiment 라벨을 **재라벨링 없이 그대로**
...
**알려진 구체적 충돌** (PR 리뷰에서 지적됨, 일반론이 아니라 실측치): 이 스크립트의
   ...
   전수 점검 — **이 충돌은 실측상 0.13%(2,314건 중 3건)이고, 그중 실제
...
**TARGET_MAP 정정**: 71630의 `신축성`·`마감`을 소재로, `길이`를
...
"핏"은 TARGET_MAP으로 "사이즈"에 통합(REVIEW_ALLOWED_ASPECTS와 스코프 일치).
같은 aspect가 리뷰 한 건 안에서 여러 번(감성 다르게) 나오면, 우리 프롬프트와 동일한
    ...
    async def one(index: int, chunk: list[dict]) -> None:
        ...
        async with semaphore:
            try:
                ...
                # classify_aspect()가 return_exceptions=True로 바뀌면서
