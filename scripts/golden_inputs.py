...
---------------------------------
`eval/README.md` 가 **"`data/golden/` 은 `eval/` 만 읽는다. `app/` 코드가 import 하면
컨닝이다"** 라고 정했다. 운영 진입점인 daily.py 가 골든 라벨을 읽지 않도록 이 모듈로
갈라 뒀다.
...
`eval/README.md` 가 실험① 에 붙인 경고가 그대로 적용된다. 실제 분류를 섞으려면
...
def load_golden_inputs(
    ...
    # **CS 와 리뷰를 둘 다 읽어야 한다.** 한쪽만 넣으면 [8] 종합이 성립하지 않아
    # source_signals 한쪽이 영원히 null 이 되고, BH family 도 절반으로 줄어 컷오프가
    # 달라진다(실측: CS 만 756검정 / 둘 다 1,464검정).
    sources = [
        (
            "cs",
