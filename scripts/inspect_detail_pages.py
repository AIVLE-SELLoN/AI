...
🔴 **기본은 현재 회사(`current_tenant()`) 문서만 내보낸다 (서영님 #84 리뷰).**
   컬렉션은 회사 여럿을 담을 수 있는데 예전엔 `where` 없이 전건을 떠서, 공유 Chroma 에서
   돌리면 **다른 회사 상세페이지 원문이 한 CSV 로 새어 나갔다.** metadata 격리는 권한
   경계가 아니므로(`docs/vectordb_tenancy.md` §5), 저장소가 주는 운영 도구라도 기본
   동작은 회사 범위를 지켜야 한다.

⚠️ `--all-companies` 는 전사 관리자용 탈출구다. 회사 축이 **없는 구형 문서**를 보려면
   이 옵션이 필요하다 — 구형은 `company_id` 가 아예 없어서 회사 필터에 안 걸린다.
