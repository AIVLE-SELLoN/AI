...
배경
----
`eval/eval_sets/relabel_300.csv`(71630 재라벨링)는 색상·사이즈·소재 3개만 커버한다
(71630이 리뷰 데이터라 파손·오배송·기타가 자연 발생하지 않음 — 리뷰는 "받아서 써본 뒤"
...
예시를 보고 생성하면 그 자체가 "정답을 아는 시험문제"가 된다 — 예시15-3/15-4가 실패사례
패턴에서 나왔던 것과 같은 유형의 실수다.
...
우연히 겹칠 위험이 있다. 3가지로 보완한다:
  ...
  3. 샘플 50~100건은 사람이 직접 읽고 검수 필요(자동화 불가 — 별도 진행)
     아래 라벨 검수 게이트가 있어도 3번은 여전히 필수다. 게이트를 통과한 문장은 "검수 모델이
     동의한 문장"이라 오히려 평가가 낙관적으로 편향된다.
...
MULTI_ASPECT_QUOTA = 100  # 2개 aspect 동시 등장(explode 계약 검증용)
...
# ── 감성 조작적 정의 ──────────────────────────────────────────────────────
# 프롬프트에 `감성: 부정` 다섯 글자만 넣으면 생성기가 매 호출마다 "부정"을 다르게 해석하고,
# 그 해석 차이가 그대로 golden label 이 된다(가정형 문의에 -1이 붙고, 실제 오배송 서술에
# 0/1이 붙는 사고). 정의를 명시해서 해석 폭을 없앤다.
...
def select_personas(sentiment: int | list[int]) -> list[str]:
    ...
    페르소나를 감성과 **독립적으로** 뽑으면 `감성=긍정 + "화가 나서 항의하는 고객"` 조합이
    나온다. 그러면 라벨은 1인데 문장은 항의문이 된다 — 실제로
...
def build_verify_prompt(text: str, aspects: list[str]) -> str:
    """생성된 문장을 **다시 읽고** 감성을 매기는 검수용 프롬프트.
...
async def generate_one(
    ...
    """1건 생성 + 중복 방지 + **라벨 검수 게이트**.

    `sample_sentiment()`가 뽑은 값을 검수 없이 그대로 라벨로 저장하면, 라벨이 "생성 결과에
    대한 관측"이 아니라 "생성 지시"가 되고 생성기가 지시를 못 지키면 그대로 오라벨이 된다.

    그래서 생성된 문장을 다시 읽혀서 요청 감성과 일치하는지 확인하고, **불일치하면 라벨을
    고치는 게 아니라 문장을 재생성**한다. 라벨을 고치면 golden이 모델 판단으로 오염되므로
    (같은 모델이 채점까지 하면 평가가 순환논리가 됨), 스펙인 라벨은 고정하고 문장을 맞춘다.

    알려진 한계: 살아남는 문장은 "검수 모델이 동의한 문장"이라 평가가 낙관적으로
...
async def run_generation(outfile: Path, seed: int, verify: bool = True) -> None:
    ...
    # ID는 **생존 행 순번**이라 내용 기반이 아니다. 검수 게이트가 실패 행을 버리므로
    # 재생성하면 GEN-#### 가 밀리고, 예전에 만든
