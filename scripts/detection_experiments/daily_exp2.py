...
덮이므로 배경·리뷰는 oracle 로 남는다 — 실험②의 계약(§9-12)과 같다.

⚠️ 캐시가 덮는 날짜만 유효하다. extend_cache.py 로 [we-8, we] 9일치를 태워두면
   연속 1·2·3일을 전부 볼 수 있다. 안 태운 날은 골든이 남아 결과가 낙관적으로 나오므로,
   아래 커버리지 검사가 부족하면 경고하고 그 조합을 건너뛴다.
"""
import csv
import json
import sys
...
def main() -> None:
    ...
    #   창 밖 발화 = 그 외 전부                                            → 헛알림
    #   헛알림 비율 = 창밖 / (창안 + 창밖)   ← v2 §5 가 "9% → 25%" 라 부른 값
    print(f"{'=' * 92}")
