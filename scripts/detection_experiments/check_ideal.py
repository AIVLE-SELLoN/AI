...
# 무거운 import 는 아래 `main()` 안에 그대로 둔다("실행할 때만" 이 이 파일 설계다). 여기로
# 올린 것은 경로 설정과 `app.core.console` 뿐인데, 그 모듈은 `contextlib`·`sys` 만 쓰고
# 부수효과가 없다. 가드가 몽키패치를 걸려면 import 가 모듈 최상단이어야 한다.
...
def main() -> None:
    ...
    for r in cfg[:6]:
        ...
        past_s, past_e = ordinal(
            we - CURRENT_WINDOW_DAYS - PAST_WINDOW_DAYS + 1
        ), ordinal(we - CURRENT_WINDOW_DAYS)
        ...
        for label, rows in sets.items():
            ...
            print(
                f"{'':38s} {label:10s} {c_n:5d}/{c_t:<6d} "
                f"{p_n:5d}/{p_t:<6d} {100 * d:7.1f}p"
            )
        print()
